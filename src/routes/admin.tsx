...
import { Settings, ShieldAlert, KeyRound, Plus, Trash, RotateCcw, Save, Sparkles, LogOut } from "lucide-react";
...
function AdminPage() {
  ...
  async function handleSave(e: React.FormEvent) {
    ...
    try {
      ...
      setStatus("Salvo localmente, mas falhou ao notificar o webhook.");
    } finally {
      setSending(false);
    }
  }
    ...
    setCfg({
      ...
      plans: [
        ...cfg.plans,
        {
          ...
          whatsappMessage: "Olá Felipe! Meu nome é {nome} e quero garantir o desconto no plano {plano}.",
        },
      ],
    });
  }
      ...
      >
        ...
        <div className="relative w-full max-w-md z-10 animate-fade-up">
          ...
          >
            ...
            >
              ...
              <div className="space-y-4">
                <div>
                  <label className="block text-xs uppercase tracking-wider text-gold-soft font-semibold mb-2" style={{ color: "var(--gold-soft)" }}>
          ...
          <p className="text-center text-xs text-white/30 mt-6 md:mt-8">
            <Link to="/" className="underline hover:text-white/60">Voltar à página de acesso público</Link>
          </p>
        </div>
      </main>
    );
      ...
      <div className="max-w-3xl mx-auto relative z-10">
        <header className="mb-10 flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-white/10 pb-6 animate-fade-up">
          <div className="flex items-center gap-4">
            <div className="h-12 w-12 rounded-2xl bg-black/50 border border-gold/30 flex items-center justify-center" style={{ boxShadow: "var(--shadow-gold)" }}>
              <Settings className="h-6 w-6 text-gold" style={{ color: "var(--gold)" }} />
            </div>
            <div>
              <h1 className="text-2xl font-bold font-display tracking-tight text-gold-gradient">Painel de Controle</h1>
              <p className="text-xs text-white/50">Personalize ofertas, chaves e webhooks para o Instagram</p>
            </div>
          </div>
        ...
        <form onSubmit={handleSave} className="space-y-8 animate-fade-up" style={{ animationDelay: "100ms" }}>
              ...
              {cfg.plans.map((plan, i) => (
                ...
                >
                  <div className="flex items-center justify-between border-b border-white/5 pb-2">
                    <span className="text-[10px] font-bold uppercase tracking-widest text-gold-soft" style={{ color: "var(--gold-soft)" }}>
                      Plano 0{i + 1}
                    </span>
                    <button 
                      type="button" 
                  ...
                  <Field label="Mensagem do WhatsApp Pré-preenchida" hint="Use {nome} e {plano} para personalizar baseado em quem preencher.">
                    <textarea 
        ...
        <footer className="mt-12 text-center text-xs text-white/30 border-t border-white/5 pt-6">
          ...
          <p className="mt-2">
            <Link to="/" className="underline hover:text-white/60">Abrir Portal Público de Vendas</Link>
          </p>
        </footer>
      </div>
    </main>
...
const inputCls = "w-full mt-1 px-4 py-2.5 rounded-xl bg-black/45 border text-sm text-white placeholder:text-white/20 focus:outline-none focus:ring-1 focus:ring-gold/30 transition-all";
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between mb-4">
        <h2 className="text-xs font-semibold uppercase tracking-widest text-gold" style={{ color: "var(--gold-soft)" }}>{title}</h2>
        {action}
      </div>
